...
function Header() {
  ...
  return (
    <div className="bg-whiteCustom header text-[36px] flex items-end justify-between ">
       ...
       */}
      <Link to="/">
        <h1 className="text-blue pt-14 pb-5 pl-6 font-pangram font-black">
          Keep<span className="text-red">'Up</span>
        </h1>
      </Link>
      ...
      {connected && (
        <div className="flex flex-col items-center font-bold text-blue text-[14px] pb-[28px] pr-10 sm:flex-row sm:text-[24px] sm:gap-5 ">
          <Link to="/team">
            <button className="group block hover:text-red focus:outline-none">
              <span className="group-focus:text-red group-focus:underline">
                About
              </span>
            </button>
          </Link>
          <button
            onClick={handleClick}
            className="group block hover:text-red focus:outline-none md:hidden"
          >
            <span className="group-focus:text-red group-focus:underline">
              Déconnexion
            </span>
          ...
          <Link
            to={'/events/maps'}
            className="flex items-center justify-end hidden md:block"
          >
            Voir la carte
          </Link>
          <Link
